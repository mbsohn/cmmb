library(doParallel); library(brglm2)
...
PCS <- function(Y, M, tr, X, n.boot=2000, sig.level=0.05, n.cores=NULL){
  c.tr <- c(scale(tr, scale=FALSE))
  pcs_rslt <- est_pcs(Y, M, c.tr, X)
  ...
  pcs.nest <- foreach(j=1:n.boot) %dopar% {
    ...
    tmp.pcs.est <- est_pcs(boot.Y, boot.M, boot.tr, boot.X)
  }
...
PCP <- function(Y, M, tr, X, n.boot=2000, sig.level=0.05, n.cores=NULL){
  ...
  registerDoParallel(n.cores)
  pcp.nest <- foreach(j=1:n.boot) %dopar% {
    indx_1 <- which(Y>0); indx_0 <- which(Y==0)
    n.1 <- length(indx_1); n.0 <- length(indx_0)
    indx <- sort(c(sample(indx_1, n.1, replace=TRUE), sample(indx_0, n.0, replace=TRUE)))
    boot.Y <- Y[indx]; boot.M <- M[indx,]; boot.tr <- c(scale(tr[indx], scale=FALSE))
    boot.X <- X[indx,,drop=FALSE]
    tmp.pcs.est <- est_pcp(boot.Y, boot.M, boot.tr, boot.X)
  }
